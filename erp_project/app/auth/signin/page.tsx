import { signIn } from "@/lib/auth"

export default async function SignInPage({
  searchParams,
}: {
  searchParams: Promise<{ callbackUrl?: string; error?: string }>
}) {
  const params = await searchParams

  return (
    <main className="min-h-screen flex items-center justify-center bg-zinc-950">
      <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-10 w-full max-w-sm">
        <h1 className="text-2xl font-bold text-zinc-100 mb-1">ERP System</h1>
        <p className="text-zinc-400 text-sm mb-8">
          Sign in with your company Google account.
        </p>

        {params.error === "AccessDenied" && (
          <p className="text-rose-400 text-sm mb-6 rounded-lg bg-rose-950 border border-rose-800 px-4 py-3">
            Your account is not authorised. Contact your administrator.
          </p>
        )}

        <form
          action={async () => {
            "use server"
            await signIn("google", {
              redirectTo: params.callbackUrl ?? "/",
            })
          }}
        >
          <button
            type="submit"
            className="w-full rounded-xl bg-white text-zinc-900 font-semibold py-3 flex items-center justify-center gap-3 hover:bg-zinc-100 transition-colors"
          >
            <svg width="20" height="20" viewBox="0 0 48 48">
              <path fill="#EA4335" d="M24 9.5c3.54 0 6.71 1.22 9.21 3.6l6.85-6.85C35.9 2.38 30.47 0 24 0 14.62 0 6.51 5.38 2.56 13.22l7.98 6.19C12.43 13.72 17.74 9.5 24 9.5z"/>
              <path fill="#4285F4" d="M46.98 24.55c0-1.57-.15-3.09-.38-4.55H24v9.02h12.94c-.58 2.96-2.26 5.48-4.78 7.18l7.73 6c4.51-4.18 7.09-10.36 7.09-17.65z"/>
              <path fill="#FBBC05" d="M10.53 28.59c-.48-1.45-.76-2.99-.76-4.59s.27-3.14.76-4.59l-7.98-6.19C.92 16.46 0 20.12 0 24c0 3.88.92 7.54 2.56 10.78l7.97-6.19z"/>
              <path fill="#34A853" d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.73-6c-2.18 1.48-4.96 2.31-8.16 2.31-6.26 0-11.57-4.22-13.47-9.91l-7.98 6.19C6.51 42.62 14.62 48 24 48z"/>
            </svg>
            Sign in with Google
          </button>
        </form>
      </div>
    </main>
  )
}
